    ...
    <ul className='bg-slate-800 p-4 rounded-lg shadow-md mb-5 flex flex-col justify-items-center items-center'>
      ...
      {postsPage.map((video) => (
        <li key={video.id} className='mb-5 max-w-xs text-center'>
          <iframe width='360' height='115' src={video.URLVideo} title={`${video.title}`} allow='accelerometer autoplay clipboard-write encrypted-media gyroscope picture-in-picture web-share' />
